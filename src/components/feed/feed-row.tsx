...
function actionText(item: GroupedFeedItem): string {
  switch (item.type) {
    ...
    case "pr":
      return item.count > 1
        ? `merged ${item.count} PRs into`
        : "merged PR into";
    case "create":
      return "created branch in";
    case "issue":
      return item.count > 1
        ? `opened ${item.count} issues in`
        : "opened issue in";
    case "streak":
      return "vibed";
    case "joined":
...
function rowModifierClass(
  type: GroupedFeedItem["type"],
  tier?: BadgeTier,
): string {
  switch (type) {
    case "project":
    ...
    case "badge":
      return tier
        ? `fl-feed-item--badge fl-feed-item--badge-${tier}`
        : "fl-feed-item--badge";
    default:
      return "";
  }
...
export function FeedRow({ item, compact = false, nowMs = null }: FeedRowProps) {
  ...
  return (
    <div
      className={`fl-feed-item ${rowModifierClass(item.type, item.badge_tier)}`}
    >
      <Link
          ...
          {item.avatar_url ? (
            ...
            <span
              style={{
                fontWeight: 600,
                fontSize: 14,
                color: isMilestone ? "#0A0A0E" : "var(--text-muted, #8A8B94)",
              }}
            >
              {initials}
            </span>
      ...
      >
        <div style={{ fontSize: 15, color: "var(--text-secondary)" }}>
            ...
            >
              {Array.from({ length: 5 }).map((_, i) => (
                <Star
                  ...
                  size={12}
                  className={
                    i < (item.rating ?? 0) ? "fl-star-filled" : "fl-star-empty"
                  }
                />
              ))}
            </span>
          ...
          {item.streak > 1 &&
            item.streak < 30 &&
            (item.type === "push" || item.type === "streak") && (
          ...
          <div className="fl-attachment fl-attachment-badge">
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <Medal
                ...
                size={16}
                style={{
                  color: `var(--badge-${item.badge_tier}, var(--accent))`,
                }}
              />
              <span
                style={{
