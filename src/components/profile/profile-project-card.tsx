  ...
  async function handleVerify() {
    ...
    try {
      ...
      if (data.verified) {
        setVerifyMessage({
          success: true,
          text: data.reason || "Project verified!",
        });
        router.refresh();
      } else {
        setVerifyMessage({
    ...
    >
      ...
      <div className="flex flex-col gap-2 p-4 flex-grow">
        <div className="flex justify-between items-start gap-3">
          <span className="text-[1.1rem] font-bold text-[var(--foreground)] min-w-0 break-words">
            {project.title}
          </span>
        ...
        <div className="flex-grow">
          <p
            className={`text-[0.9rem] text-[var(--text-secondary)] font-medium ${isLongDescription && !expanded ? "line-clamp-4" : ""}`}
          >
            {project.description}
          </p>
          {isLongDescription && (
        ...
        {reportStatus === "success" && (
          <p className="text-xs font-medium text-green-700 mt-2">
            Thanks for the report! We&apos;ll review this project shortly.
          </p>
        )}
        {reportStatus === "error" && (
        ...
        {reportStatus === "auth" && (
          <p className="text-xs font-medium text-red-600 mt-2">
            Please sign in to report a project.
          </p>
        )}
      </div>
    </div>
