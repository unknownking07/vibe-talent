...
export function LeaderboardRow(p: LeaderboardRowProps) {
  return (
    <Link
      ...
      className={`grid items-center gap-5 px-5 py-4 border-b border-[var(--border-subtle)] last:border-b-0 hover:bg-[var(--bg-surface-light)] transition-colors ${p.isCrown ? "bg-[var(--status-warning-bg)] border-b-[var(--border-hard)]" : ""}`}
      style={{ gridTemplateColumns: "44px 48px 1fr auto auto" }}
    >
      <span className="font-mono text-[15px] font-bold text-[var(--text-secondary)]">
