...
- $VIBE Token (contract address, where to buy, utility): https://www.vibetalent.work/token
...
- **Chain**: Solana
- **Decimals**: 9
- **Where to buy**: https://bags.fm/FfDYT3WqimMw7itMxw4kYJ26GPG78RfpZmepQCFpBAGS. Buy with SOL or USDC on Solana. $VIBE trades on a Bags bonding curve and is NOT routable on Jupiter, so it cannot be swapped through Jupiter-based aggregators.
- **Token page**: https://www.vibetalent.work/token
...
- **Two ways to prove a wallet**: (1) sign a server-issued nonce, a message signature that cannot move funds and is the recommended route; or (2) for builders who will not connect a deployer wallet to a website, send one transaction from that wallet to itself carrying a server-issued memo, which VibeTalent watches for on-chain. The memo names the site, the account the wallet would be linked to, and a single-use challenge, so the transaction states in plain words what it authorises and who it is for. Either way the wallet never grants VibeTalent any spending permission. Neither route can stop someone who is tricked into signing anyway: the memo makes the intent legible, it is not a guarantee against phishing.
