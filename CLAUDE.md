...
- **Two ways to prove a Solana wallet, and they are not interchangeable.** The default is a message signature over a server-issued nonce (`/api/wallet/link`) — it cannot move funds. The fallback, for builders who will not connect a deployer wallet, is `/api/wallet/verify-transfer`: they send an exact, randomly-chosen lamport amount (well under a cent) to `CHAIN_CONFIGS.solana.receivingWallet`, and we watch the chain for it. **The amount, not a memo, is the binding.** The first cut was memo-only because a memo is a sentence the signer can read, but Phantom and most Solana wallets cannot attach one, so it was unusable in practice — an unusable proof just pushes people back to connecting. A memo is still accepted when a wallet can send one. The amount is reserved while a challenge is open and the transaction must postdate it, so two challenges cannot share a number and an older transfer cannot be replayed to claim its sender's wallet. **Neither route is phishing-proof**: an amount carries no intent, so a relayed "send exactly X to Y" can still credit an attacker. The residual risk is bounded by key control — whoever holds the key can prove the wallet again, so a stolen attribution is recoverable.
- **Attribution follows the proof.** Linking a wallet claims that wallet's `bags_launches` rows; unlinking releases them. Without this, a builder who unlinks keeps being shown as the proven creator of coins from a wallet they no longer control.
