...
- **Two ways to prove a Solana wallet, and they are not interchangeable.** The default is a message signature over a server-issued nonce (`/api/wallet/link`) — it cannot move funds. The fallback for builders who will not connect a deployer wallet is `/api/wallet/verify-transfer`: they send one transaction from that wallet **to itself** carrying a server-issued memo, and we watch the chain for it. The memo is what makes it defensible — a bare "send X lamports to this address" scheme is trivially relayable, because the payer sees an amount and an address but no statement of intent, so an attacker can start a challenge and phish the real owner into satisfying it. The memo reads "Link wallet to vibetalent.work for @<username> | <nonce>", naming the account so a victim can see it is not theirs. **This raises the bar, it does not remove it**: no challenge-response scheme without a second channel stops someone who signs anyway, and that is equally true of the signature route. Never replace the memo with a unique-amount scheme, and never describe either route as phishing-proof.
